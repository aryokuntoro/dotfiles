...
# Gets battery percentage of a device, if it reports one
device_battery() {
    device_info=$(bluetoothctl info "$1")
    echo "$device_info" | grep -oP 'Battery Percentage:.*\(\K[0-9]+(?=\))'
}
...
show_menu() {
    ...
    if power_on; then
        ...
        # Connected devices show their battery percentage, if available
        devices=""
        while IFS= read -r line; do
            dev_mac=$(echo "$line" | awk '{print $2}')
            dev_name=$(echo "$line" | cut -d ' ' -f 3-)
            if device_connected "$dev_mac"; then
                dev_battery=$(device_battery "$dev_mac")
                [ -n "$dev_battery" ] && dev_name="$dev_name (${dev_battery}%)"
            fi
...
rofi_command="rofi -theme ~/.config/rofi/themes/Bluetooth.rasi -dmenu $* -p"
