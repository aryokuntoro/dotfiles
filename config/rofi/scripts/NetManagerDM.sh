...
def create_wifi_profile(nm_ap, password, adapter):
    ...
    if ap_sec != "--":
        ...
        if "WPA" in ap_sec:
            if "WPA3" in ap_sec and "WPA2" not in ap_sec:
                s_wifi_sec.set_property(
                    NM.SETTING_WIRELESS_SECURITY_KEY_MGMT, "sae"
                )
            else:
                # WPA2-only or WPA2/WPA3-transitional: wpa-psk is the more
                # broadly compatible choice. Forcing pure "sae" on a mixed
                # AP can fail the handshake even with the correct password.
                s_wifi_sec.set_property(
