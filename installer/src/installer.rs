    ...
    for item in items.iter().filter(|i| i.checked) {
        ...
        let result = match &item.action {
            Action::ConfigDir { name, source } => {
                copy_one(source, &home.join(".config").join(name), home, &backup_dir, &mut prompter)
            }
...
fn install_home_file(
    ...
    copy_one(source, &target, home, backup_dir, prompter)
}
...
fn install_applications(
    ...
    for entry in std::fs::read_dir(source_dir)? {
        ...
        if matches!(copy_one(&entry.path(), &target_dir.join(&name), home, backup_dir, prompter)?, Applied::Skipped) {
            any_skipped = true;
        }
    }
...
/// The home directory baked into this repo's own copy of a handful of
/// config files whose format has no portable way to reference "the
/// current user's home" -- Thunar's uca.xml <command> paths, the
/// mount-image.desktop Exec= line, qt5ct/qt6ct's color_scheme_path,
/// greenclip.toml's history_file, uad's backup_folder, and the GTK
/// bookmarks file. Those were saved by whichever tool owns the format
/// (Thunar's UI, qt5ct's file picker, ...), not generated by a script
/// the way apply-theme-colors.sh's output is, so they can't self-correct
/// at runtime -- rewritten to the installing user's real $HOME on copy
/// instead, in copy_file_substituting_home below.
const BAKED_IN_HOME: &str = "/home/kiddo";

/// Copies one file, rewriting any occurrence of BAKED_IN_HOME to `home`
/// if the file is UTF-8 text. Binary files (GTK/cursor theme assets
/// under install_gtk_theme's copy_dir_recursive calls, mainly) fail the
/// UTF-8 check and fall through to a plain byte-for-byte copy, same as
/// before this existed.
fn copy_file_substituting_home(source: &Path, target: &Path, home: &Path) -> io::Result<()> {
    let bytes = std::fs::read(source)?;
    if let Ok(text) = std::str::from_utf8(&bytes) {
        if text.contains(BAKED_IN_HOME) {
            let home_str = home.to_str().expect("$HOME must be valid UTF-8");
            return std::fs::write(target, text.replace(BAKED_IN_HOME, home_str));
        }
    }
...
fn copy_one(source: &Path, target: &Path, home: &Path, backup_dir: &Path, prompter: &mut Prompter) -> io::Result<Applied> {
    ...
    if source.is_dir() {
        copy_dir_recursive(source, target, home)?;
        mark_scripts_executable(target)?;
    } else {
        copy_file_substituting_home(source, target, home)?;
    }
    Ok(Applied::Done)
}
...
fn copy_dir_recursive(source: &Path, target: &Path, home: &Path) -> io::Result<()> {
    ...
    for entry in std::fs::read_dir(source)? {
        ...
        if file_type.is_symlink() {
            ...
            copy_dir_recursive(&from, &to, home)?;
        } else {
            copy_file_substituting_home(&from, &to, home)?;
        }
    }
    Ok(())
}
...
fn install_gtk_theme(theme: &crate::items::GtkTheme, home: &Path, tx: &Sender<Progress>) -> io::Result<()> {
    ...
    match &theme.build {
        BuildMethod::PlainCopy => {
            ...
            copy_dir_recursive(&clone_dir, &dest, home)?;
...
mod tests {
    ...
    /// Files like Thunar's uca.xml, qt5ct.conf and greenclip.toml are saved
    /// by their own app with this repo author's literal home directory
    /// baked in -- on a different account that path doesn't exist, so
    /// e.g. every Thunar custom action or the qt5ct color scheme would
    /// silently point nowhere. copy_one/copy_dir_recursive must rewrite
    /// BAKED_IN_HOME to the real installing $HOME, for both a loose file
    /// (ConfigFile) and a file inside a copied directory (ConfigDir),
    /// while leaving content that doesn't mention it untouched.
    #[test]
    fn baked_in_home_is_rewritten_to_the_real_home() {
        let tmp = std::env::temp_dir().join(format!(
            "dotfiles-installer-hometest-{}",
            SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()
        ));
        let fake_repo = tmp.join("repo");
        let fake_home = tmp.join("home");
        std::fs::create_dir_all(fake_repo.join("config/rofi/scripts")).unwrap();
        std::fs::create_dir_all(&fake_home).unwrap();

        std::fs::write(
            fake_repo.join("config/greenclip.toml"),
            format!("history_file = \"{BAKED_IN_HOME}/.cache/greenclip.history\"\n"),
        )
        .unwrap();
        std::fs::write(
            fake_repo.join("config/rofi/scripts/unrelated.sh"),
            "#!/usr/bin/env bash\necho no path here\n",
        )
        .unwrap();
        ...
        let items = vec![
            Item {
                label: "config/greenclip.toml".into(),
                description: String::new(),
                category: Category::Config,
                checked: true,
                action: Action::ConfigFile {
                    name: "greenclip.toml".into(),
                    source: fake_repo.join("config/greenclip.toml"),
                },
            },
            Item {
                label: "config/rofi".into(),
                description: String::new(),
                category: Category::Config,
                checked: true,
                action: Action::ConfigDir { name: "rofi".into(), source: fake_repo.join("config/rofi") },
            },
        ];
        ...
        let handle = {
            ...
            let cancel = AtomicBool::new(false);
            thread::spawn(move || run(&items, &fake_home, &tx, &cancel, &answer_rx))
        };
        for p in rx.iter() {
        ...
        assert_eq!(
            std::fs::read_to_string(fake_home.join(".config/greenclip.toml")).unwrap(),
            format!("history_file = \"{home_str}/.cache/greenclip.history\"\n"),
            "the loose ConfigFile copy should have rewritten BAKED_IN_HOME"
        );
        assert_eq!(
            std::fs::read_to_string(fake_home.join(".config/rofi/scripts/unrelated.sh")).unwrap(),
            "#!/usr/bin/env bash\necho no path here\n",
            "a file with no BAKED_IN_HOME mention must come through byte-for-byte unchanged"
        );
